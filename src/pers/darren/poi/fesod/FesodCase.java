...
public class FesodCase {

    private static final List<User> USER_LIST = Lists.newArrayList();
    private static final String FILE_PATH = "/home/darren/Temporary/fesod/用户列表.xlsx";
    ...
    private static void simpleWriteExcel() {
        FesodSheet.write(FILE_PATH, User.class).sheet("用户列表").doWrite(USER_LIST);
    }

    private static void simpleReadExcel() {
        // 自定义ReadListener
        FesodSheet.read(FILE_PATH, User.class, new ReadListener<User>() {
        ...
        FesodSheet.read(FILE_PATH, User.class, new PageReadListener<User>(pageList -> {
        ...
        FesodSheet.read(FILE_PATH, User.class, new PageReadListener<User>(10, pageList -> {
            System.out.println("分页列表大小：" + pageList.size());
            System.out.println("分页列表数据：" + pageList);
            System.out.println("分页列表数据逐条打印：");
            pageList.forEach(System.out::println);
        })).sheet().doRead();
        // pers.darren.poi.fesod.PageReadListener构造方法参数为null检查
        try {
            FesodSheet.read(FILE_PATH, User.class, new PageReadListener<User>(null, pageList -> {
            })).sheet().doRead();
        } catch (Exception e) {
        ...
        // pers.darren.poi.fesod.PageReadListener构造方法参数为null检查
        try {
            FesodSheet.read(FILE_PATH, User.class, new PageReadListener<User>(10, null)).sheet().doRead();
        } catch (Exception e) {
            System.err.println(e.getMessage());
        }
    }
